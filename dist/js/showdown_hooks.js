...
function get_box() {
    ...
    for (i in names) {
        if (names[i].includes("My Box")) {
            ...
            var pok_name = names[i].split(" (")[0].toLowerCase().replace(" ","-").replace(".","").replace(".","").replace("’","").replace("-totem","")
...
function get_trainer_preview(poks) {
  ...
  for (i in poks) {
    pok = poks[i]
    var pok_name = pok.split(" (")[0].toLowerCase().replace(" ","-").replace(".","").replace(".","").replace("’","").replace("-totem","")
    var pok_html = `<img class="trainer-pok right-side" src="./img/pokesprite/${pok_name}.png" data-id="${poks[i].split("[")[0]}">`
...
function getSet(setName, selector) {
  ...
  if (pokemon) {
    ...
    if (randset) {
      ...
      if (gen >= 3) $(selector).closest('.poke-info').find(".ability-pool").show();
      $(selector).closest('.poke-info').find(".extraSetAbilities").text(listAbilities.join(', '));
      if (gen >= 2) $(selector).closest('.poke-info').find(".item-pool").show();
      $(selector).closest('.poke-info').find(".extraSetItems").text(listItems.join(', '));
      if (gen >= 9) {
      ...
      $(selector).closest('.poke-info').find(".extraSetRoles").text(listRoles.join(', '));
      var listTeraTypes = [];
      ...
      $(selector).closest('.poke-info').find(".extraSetTeraTypes").text(listTeraTypes.join(', '));
    } else {
    ...
    if (regSets || randset) {
      ...
      if (randset) {
        ...
        $(selector).closest('.poke-info').find(".extraSetMoves").html(formatMovePool(setMoves));
      }
    } else {
