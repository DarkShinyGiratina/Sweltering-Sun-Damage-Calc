...
function calculateSMSSSV(gen, attacker, defender, move, field) {
    ...
    else if (move.named("Ancient Force") &&
        attacker.item &&
    ...
    var noTypeChange = move.named("Revelation Dance", "Ancient Force", "Nature Power", "Techno Blast", "Multi Attack", "Natural Gift", "Weather Ball", "Terrain Pulse") ||
    ...
    if (move.named("Cosmic Chaos")) {
        var lostHP = field.defenderSide.isProtected
            ? 0
    ...
    else if (attacker.hasAbility("Protean", "Libero", "Versatility") &&
        !attacker.teraType) {
        stabMod += 2048;
        desc.attackerAbility = attacker.ability;
    }
    ...
    if (attacker.hasAbility("Expertise") && move.hits === 1 && !isSpread) {
        var child = attacker.clone();
        child.ability = "Expertise (Second Hit)";
        (0, util_2.checkMultihitBoost)(gen, child, defender, move, field, desc);
        childDamage = calculateSMSSSV(gen, child, defender, move, field)
            .damage;
        desc.attackerAbility = attacker.ability;
...
function calculateBasePowerSMSSSV(gen, attacker, defender, move, field, hasAteAbilityTypeChange, desc) {
    ...
    switch (move.name) {
        ...
        case "Triple Axel":
            ...
            break;
        case "Triple Kick":
            basePower = move.hits === 2 ? 20 : move.hits === 3 ? 30 : 10;
            desc.moveBP = basePower;
            break;
        case "Crush Grip":
...
function calculateBPModsSMSSSV(gen, attacker, defender, move, field, desc, basePower, hasAteAbilityTypeChange, turnOrder) {
    ...
    var resistedKnockOffDamage = !defender.item ||
        ...
        (defender.name.includes("Giratina-Origin") &&
            defender.item.includes("Griseous")) ||
    ...
    if ((move.named("Facade") && attacker.hasStatus("brn", "par", "psn", "tox")) ||
        ...
        var type2Effectiveness = types[1]
            ? (0, util_2.getMoveEffectiveness)(gen, move, types[1], isGhostRevealed, field.isGravity, isRingTarget)
            : 1;
    ...
    var isUserAuraBreak = attacker.hasAbility("Aura Break") ||
...
function calculateAttackSMSSSV(gen, attacker, defender, move, field, desc, isCritical) {
    ...
    var attack;
    var attackSource = move.named("Foul Play") ? defender : attacker;
    if (move.named("Photon Geyser", "Light That Burns The Sky", "Twilight Cannon") ||
        (move.named("Tera Blast") && attackSource.teraType)) {
        move.category =
            attackSource.stats.atk > attackSource.stats.spa ? "Physical" : "Special";
    }
...
function calculateAtModsSMSSSV(gen, attacker, defender, move, field, desc) {
    ...
    else if ((attacker.hasAbility("Water Bubble", "Liquid Veil") &&
        move.hasType("Water")) ||
    ...
    if ((defender.hasAbility("Thick Fat") && move.hasType("Fire", "Ice")) ||
        (defender.hasAbility("Water Bubble", "Liquid Veil") &&
            move.hasType("Fire")) ||
    ...
    if ((attacker.hasAbility("Hadron Engine") &&
        move.category === "Special" &&
...
function calculateDfModsSMSSSV(gen, attacker, defender, move, field, desc, isCritical, hitsPhysical) {
    ...
    else if (defender.hasAbility("Grass Pelt", "Grassy Guard") &&
        field.hasTerrain("Grassy") &&
        hitsPhysical) {
        dfMods.push(6144);
        desc.defenderAbility = defender.ability;
...
function calculateFinalModsSMSSSV(gen, attacker, defender, move, field, desc, isCritical, typeEffectiveness) {
    ...
    else if ((attacker.hasAbility("Sniper") || attacker.hasAbility("Ruthless")) &&
        isCritical) {
        finalMods.push(6144);
        desc.attackerAbility = attacker.ability;
    }
    ...
    if (defender.hasAbility("Solid Rock", "Filter", "Prism Armor", "Resilient") &&
        typeEffectiveness > 1) {
        finalMods.push(3072);
        desc.defenderAbility = defender.ability;
    }
