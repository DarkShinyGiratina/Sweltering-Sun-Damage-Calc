...
export class Move implements State.Move {
  ...
  constructor(
    ...
    let data: I.Move = extend(
      true,
      { name },
      gen.moves.get(toID(name)),
      options.overrides
    );
    ...
    if (options.useZ && data.zMove?.basePower) {
      const zMoveName: string = getZMoveName(
        data.name,
        data.type,
        options.item
      );
      ...
      data = extend(true, {}, zMove, {
        name: zMoveName,
        basePower:
          zMove!.basePower === 1 ? data.zMove.basePower : zMove!.basePower,
        category: data.category,
      });
    } else {
      if (data.multihit) {
    ...
    if (
      data.self?.boosts &&
      data.self.boosts[stat] &&
      data.self.boosts[stat]! < 0
    ) {
    ...
    this.isCrit =
      ...
      (gen.num === 1 &&
        ["crabhammer", "razorleaf", "slash", "karate chop"].includes(data.id));
    ...
    if (!this.bp) {
      ...
      if (
        ["return", "frustration", "pikapapow", "veeveevolley"].includes(data.id)
      ) {
        this.bp = 102;
      }
    }
...
export function getZMoveName(
  ...
  if (moveName === "Thunderbolt" && item === "Pikashunium Z")
    return "10,000,000 Volt Thunderbolt";
  if (moveName === "Plasma Shot" && item === "Aloraichium Z")
